...
\begin{exercise}
 ...
 %  \begin{align}
 %    E^{(3)} = \sum_{iji'a'}
 %    \frac{\braket{\Phi|\hat{L}|\Phi_i^a}\braket{\Phi_i^a|\hat{L}|\Phi_{i'}^{a'}}\braket{\Phi_{i'}^{a'}|\hat{L}|\Phi}}{(\kappa_i-\kappa_a)(\kappa_{i'}-\kappa_{a'})}
 % + \frac{1}{4}\sum_{i'a'}\sum_{ijab} 
 %    + \sum_{XY} \frac{1}{\Delta \epsilon_X \Delta \epsilon_Y} \bra{\Phi} \hat{L} \ket{\Phi_X}\bra{\Phi_X} \hat{L}
 %    \ket{\Phi_Y}\bra{\Phi_Y} \hat{L}\ket{\Phi} -
 %    E^{(1)}\left(\sum_{ia}
 %  \frac{|\braket{\Phi_i^a|\hat{L}|\Phi}|^2}{(\epsilon_i - \epsilon_a)^2} +
 %  \frac{1}{4}\sum_{ijab}
 %  \frac{|\braket{\Phi_{ij}^{ab}|\hat{L}^{(2)}|\Phi}|^2}{(\epsilon_i +
 %    \epsilon_j - \epsilon_a - \epsilon_b)^2} \right)
 %  \end{align}
\end{exercise}
...
% The special case of the Hartree--Fock partitioning of the Hamiltonian
% is referred to as \emph{Møller--Plesset perturbation theory}
% (MPPT). The $n$-th order MPPT is often referred to as MP$n$.

% Writing
% \begin{equation}
%   \hat{H} = \hat{F} + \hat{U},
% \end{equation}
% where $\hat{F}$ is the Fock operator and $\hat{U}$ is the fluctiation
% potential, and setting $\hat{K} = \hat{F}$, $\hat{L} = \hat{U}$ in
% RSPT for the ground state, we notice immediately that in
% Eq.~\eqref{eq:mbpt2}, the singles-term vanishes identically,
% \begin{equation}
%   \braket{\Phi_i^a|\hat{U}|\Phi} = \braket{\Phi_i^a|\hat{H} -
%     \hat{F}|\Phi} = - \braket{\phi_a|\hat{F}|\phi_i} = 0,
% \end{equation}
% assuming that the single-particle functions solve the canonical HF
% equation~\eqref{eq:hf-can}. Moreover,
% \begin{equation}
%   E^{(1)} = \braket{\Phi|\hat{H}|\Phi} = \frac{1}{2}\sum_{ij}
%   \braket{\phi_i\phi_j|\hat{U}|\phi_i\phi_j}_\text{AS}.
% \end{equation}
